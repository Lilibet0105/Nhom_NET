...
namespace QuanLyCongViec.GUI
{
    public partial class frmPerformanceDashboard : Form
    {
        ...
        private void LoadDashboardData()
        {
            ...
            CaiDatDataGridViewBaoCao();
            CalculateSummaryCards(dtStaff);
            BuildColumnChart(dtStaff);
        }

        private void CaiDatDataGridViewBaoCao()
        {
            UiHelper.StyleGrid(dgvDetails);
            if (dgvDetails.Columns["HoTen"] != null) dgvDetails.Columns["HoTen"].HeaderText = "Nhân sự";
            if (dgvDetails.Columns["SoViec_Done"] != null) dgvDetails.Columns["SoViec_Done"].HeaderText = "Hoàn thành";
            if (dgvDetails.Columns["SoViec_InProgress"] != null) dgvDetails.Columns["SoViec_InProgress"].HeaderText = "Đang làm";
            if (dgvDetails.Columns["SoViec_ToDo"] != null) dgvDetails.Columns["SoViec_ToDo"].HeaderText = "Cần làm";
            if (dgvDetails.Columns["SoViec_TreHan"] != null) dgvDetails.Columns["SoViec_TreHan"].HeaderText = "Trễ hạn";
        }
